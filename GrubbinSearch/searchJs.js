...
//vitamin k is in dv 
require(['mongoose'], function (foo) {
    var mongoose = foo; 
    mongoose.connect('mongod://localhost:27017/GrubbinGroceries');
    ...
    $("#search").click(async function() {   
        ...
        for (i in foodObjs){
            ...
            await $.ajax(settings).done(function (response) {
            	if (Food.findOne({ name: foodObjs[i]})) 
            	{
            		$("#response").append(foodObjs[i] +" has already been added.");
            	}
            	else {
            		   var food = new Food({
	   				   name: 		foodObjs[i],       	
	                ...
	                food.save(function(err, food) {
	      				$("#response").append(food.name +" was saved.");  
	   				});
